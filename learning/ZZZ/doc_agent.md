# doc_agent.md — ZZZ 文档写作规范（AI 整理文档的要求）

> 适用范围：`learning/ZZZ/` 下所有文档（baseline / agent / basics）。
> 目的：让文档**对人类易读、负担小**，同时保持结构化以便 AI 后续增补。
> 约束：AI 在整理、重写、新增本目录文档时，必须遵守本规范。
>
> 与 PENN 的关系：结构与其规范对齐，**内容一律自写**——不复制他人笔记里的结论、
> 实测数字与个人判断；引用共享事实（比赛规则、评分公式、代码位置）时需自行核对后再落笔。
...
> 它是整个 ZZZ 学习工作的"总控台"，所有 AI 协作者必须把它当**最重要的入口文档**对待。
...
4. 它是根 README 与各模块 README 之外的"动态真相"：
   - 笔记讲"怎么理解"；`00-progress.md` 讲"现在到哪了、下一步干嘛"。
5. 其他文档交叉引用都指向这份文件，不再建平行的"进展/日志"类目录。
...
- **内容文档用中文短名**，贴主题即可（如 `架构总览.md`、`函数调用.md`）。
...
文档尚未写完时，用统一标记，避免误读为"已有结论"：

| 标记 | 含义 |
| --- | --- |
| ⏳ 待补写 | 只有提纲，还没读/没跑，待自己验证后填 |
| ✍️ 初稿 | 已写主体，结论待验证或待补充实测数据 |
| ✅ 完成 | 已验证（跑通/对照过代码），可当依据引用 |

> 规则：**⏳ / ✍️ 状态的结论不得写进 `00-progress.md` 的"成果"栏**，只能进"待办"。
...
6. 不复制他人 `learning/` 目录下的笔记内容；共享事实（规则、评分公式、命令）自己核对后再写。
