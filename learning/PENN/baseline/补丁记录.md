...
| P9 | `run/runner.py` + `cli.py` | `run-benchmark` 只能按 `--limit` 取前 N 题，无法只测 hard | 新增 `--difficulty` 选项（数据集层 `iter_tasks(difficulty=...)` 本就支持，CLI 未暴露）；`summary.json` 记录 `difficulty` 字段 |
...
## P9 详述（2026-09-06，为 hard 专项测试而加）

- 需求：换 `qwen3.6-flash` 后要**只跑 11 道 hard**，官方 CLI 没有难度过滤；
- 改动：`runner.run_benchmark(...)` 增加 `difficulty` 参数 → `dataset.iter_tasks(difficulty=difficulty)`；
  `cli.run_benchmark_command` 增加 `--difficulty TEXT`，并用同一过滤算进度条总数（否则进度条会按 50 题算）；
- 用法：`uv run dabench run-benchmark --config configs/qwen36_flash.yaml --difficulty hard`；
- 兼容：`difficulty=None` 时行为与原来完全一致（全量）。
