...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig) -> dict[str, Any]:
    ...
    # 注意：不能先 join() 再 get()。子进程向队列写入较大的结果时，
    # 若管道缓冲写满会阻塞，而父进程 join 又等待子进程退出，会形成死锁
    # （表现就是“模型已答完但任务永不结束、不写文件”）。因此先轮询队列。
    deadline = perf_counter() + timeout_seconds
    payload: dict[str, Any] | None = None
    while payload is None and perf_counter() < deadline and process.is_alive():
        try:
